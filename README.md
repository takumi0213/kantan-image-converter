...
Webページの画像を右クリックでJPG・PNG・WebPに変換して保存できるブラウザ拡張機能です（Chrome・Firefox対応）。
...
[Chrome Web Store](https://chromewebstore.google.com/detail/lecilkeobjibofjaoadlelgfiipicdlo) または [Firefox Add-ons](https://addons.mozilla.org/ja/firefox/addon/img-convert/) からインストールできます。
...
> [!NOTE]
> `chrome://`・`moz-extension://` や拡張機能ページ等の特殊なページでは変換できないため、元の形式のまま保存されます。
