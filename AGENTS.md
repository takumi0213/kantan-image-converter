...
**かんたん画像変換** は、Webページの画像を右クリックメニューからJPG・PNG・WebPに変換して保存できるブラウザ拡張機能です（Chrome・Firefox対応）。

- **種別**: ブラウザ拡張機能（Manifest V3、Chrome・Firefox対応）
...
- **Worker の拡張機能 ID 制限**: env 変数 `ALLOWED_EXTENSION_ORIGIN`（Chrome）/ `ALLOWED_GECKO_EXTENSION_ORIGIN`（Firefox）に各拡張機能の Origin を設定することで特定 ID に限定できる（未設定時はスキーム一致のみ）
- **Worker の `params` 型チェック**: `in` 演算子の TypeError を防ぐため `!params || typeof params !== "object" || Array.isArray(params)` を 400 で弾く
