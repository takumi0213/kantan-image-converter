...
`chrome://extensions` の拡張機能管理画面やコンソールに表示される場合がありますが、**無視してください**。

Firefox の AMO（addons.mozilla.org）バリデーターが `background.service_worker` に対して `background.scripts` フォールバックの併記を必須としているため、意図的に両方を記載しています。Chrome 側の警告より AMO 提出要件を優先しており、動作への影響はありません。
