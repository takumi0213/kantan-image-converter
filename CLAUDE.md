...
- **Worker の CORS 対応**: `Content-Type: application/json` の POST にはプリフライト（OPTIONS）が発生する。Worker は OPTIONS に `Access-Control-Allow-*` を返し、POST レスポンスにも CORS ヘッダーを付与する
...
- **`downloadOriginal` の `blob:` 分岐は `throw` のみ**: `showError` を呼ばず例外のみスローする。エラー表示は呼び出し元の `catch` で一元化し二重表示を防ぐ
