...
- **Worker の Origin 制限**: `chrome-extension://` スキームの Origin のみ受け付け、第三者からの濫用を防止する。さらに env 変数 `ALLOWED_EXTENSION_ORIGIN` に自拡張機能の Origin（`chrome-extension://{ID}`）を設定することで特定拡張機能 ID に限定できる（未設定時はスキーム一致のみ）
